...
// Query Expressions
query_expr : select_statement (set_op select_statement)* order_clause? limit_clause?
		   | '(' query_expr ')'
		   ;
...
expression_list : ( ( star_expr (except_statement)? (replace_statement)? ) | alias_expr ) ( ',' ( ( star_expr (except_statement)? (replace_statement)? ) | alias_expr ) )*;

star_expr: (expr '.')? '*' ;

alias_expr : expr (AS? alias_name)?;
...
skip_rows : number;
//with_query_name : ;
// WITH statement (With_Visitor statement)
with_statement : WITH cte_expr (',' cte_expr)* ;

cte_expr : cte_name AS '(' query_expr  ')' ;
